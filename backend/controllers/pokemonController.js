...
} = require("./pokemonControllerUtils");
...
const getPokemon = async (req, res) => {
  ...
  try {
    ...
    const detailedPokemonList = await addDataToPokemonList(
...
const getMatchingPokemon = async (req, res) => {
  ...
  try {
    ...
    if (err.response) {
      ...
      res
        .status(500)
        .json({ error: "There was an error fetching the Pokémon data." });
    }
  }
};
...
const getPokemonTypes = async (req, res) => {
  ...
  try {
    ...
    const lexicallySortedTypes = response.data.results.sort((a, b) =>
      a.name.localeCompare(b.name)
    );
    res.json(lexicallySortedTypes);
  } catch (err) {
    console.error(err);
    if (err.response) {
      res
        .status(err.response.status)
        .json({ error: "Pokémon types not found." });
    } else {
      res
        .status(500)
        .json({ error: "There was an error fetching the Pokémon types." });
    }
  }
};
  ...
  try {
    if (offset === 0) {
      ...
      if (sort === "likes" || sort === "dislikes") {
        if (sort === "likes") {
          const likedPokemons = await pokemonService.sortByLikes();
          pokemonList = likedPokemons.filter((name1) =>
            pokemonList.some((name2) => name1.name === name2.name)
          );
        } else if (sort === "dislikes") {
          const dislikedPokemons = await pokemonService.sortByDislikes();
          pokemonList = dislikedPokemons.filter((name1) =>
            pokemonList.some((name2) => name1.name === name2.name)
          );
        }
      }
